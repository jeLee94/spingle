...
public class Serial : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        */
        ConnectSerial();
        //StartCoroutine(GetDataFromQueue());
    ...
    {
        string[] strToSplit = {"a,", ",b\r\n"};
        //Debug.Log(queue.Count());
        //var tmp = sp.ReadExisting().Split('a','b','\r','\n');
        tmp += sp.ReadExisting();//.Split(strToSplit, System.StringSplitOptions.RemoveEmptyEntries);
                                     //Debug.Log(tmp);
        if(tmp.Count() >= 30)
        {
            var tmparr = tmp.Split(strToSplit, System.StringSplitOptions.RemoveEmptyEntries);
            foreach (string s in tmparr)
            {
                if (s.Count() == 24)
                {
                    queue.Enqueue(s);
                    /*Debug.Log("24이상 : " + s);
                    Debug.Log("가져감 : " + count1);
                    count1++;*/
                    if (queue.Any())
                    {
                        str = queue.Dequeue();
                        var splited = str.Split(',');
                        data = Array.ConvertAll(splited, int.Parse);
                        Inputdata.index_F = data[0];
                        Inputdata.mid_F = data[1];
                        Inputdata.ring_F = data[2];
                        Inputdata.little_F = data[3];
                        Inputdata.thumb = data[4];
                    }
                    //Debug.Log(queue.Count());
                }
                else

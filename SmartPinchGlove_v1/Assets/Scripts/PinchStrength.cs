...
public class PinchStrength : MonoBehaviour
{
    ...
    IEnumerator PlaySanta(float duration) // duration의 값(초) 동안 실행하는 코루틴 https://wergia.tistory.com/24 
    {
        ...
        while (maxPowerTimer < duration)    //while문 ***안에 yield return null; 들어가야함
        {
            ...
            pinchDatas.Add(new PinchData(GameManager.instance.GetInputData(), maxPowerTimer));
            ...
            if(maxPowerTimer >= 3f)
            {
                ...
                if (GameManager.instance.GetInputData() < 10) //(Input.GetKeyDown(KeyCode.K)) 
                {
        ...
        else
        {
            ...
            //DB 넣는 구간 추가
            string query = "INSERT INTO measurement (date,userID,gameID,maxPower,risingTime,releaseTime) VALUES ('" + DateTime.Now.ToString("yyyy년 MM dd일 mm분 ss초") + "','" + Data.instance.userID + "','" + "01M" + "','" + Data.instance.maxPower_average + "','" + Data.instance.risingTime + "','" + Data.instance.releaseTime + "')";
